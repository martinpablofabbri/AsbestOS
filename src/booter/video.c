...
void draw_health (unsigned health, unsigned left_offset) {
  SET_PIX(left_offset + 1, 1, 'H', BLACK, GREEN);
  SET_PIX(left_offset + 2, 1, 'e', BLACK, GREEN);
  SET_PIX(left_offset + 3, 1, 'a', BLACK, GREEN);
  SET_PIX(left_offset + 4, 1, 'l', BLACK, GREEN);
  SET_PIX(left_offset + 5, 1, 't', BLACK, GREEN);
  SET_PIX(left_offset + 6, 1, 'h', BLACK, GREEN);
  SET_PIX(left_offset + 7, 1, ':', BLACK, GREEN);
  SET_PIX(left_offset + 8, 1, ' ', BLACK, GREEN);

  if (health >= 100) {
    SET_PIX(left_offset + 9, 1, (unsigned char)((int)('0') + (health / 100)), BLACK, GREEN);
  } else {
    SET_PIX(left_offset + 9, 1, ' ', BLACK, GREEN);
  }
  if (health >= 10) {
    SET_PIX(left_offset + 10, 1, (unsigned char)((int)('0') + ((health%100) / 10)), BLACK, GREEN);
  } else {
    SET_PIX(left_offset + 10, 1, ' ', BLACK, GREEN);
  }
  SET_PIX(left_offset + 11, 1, (unsigned char)((int)('0') + (health%10)), BLACK, GREEN);
}

void draw_score (unsigned score, unsigned left_offset) {
  SET_PIX(left_offset + 1, 2, 'S', BLACK, GREEN);
  SET_PIX(left_offset + 2, 2, 'c', BLACK, GREEN);
  SET_PIX(left_offset + 3, 2, 'o', BLACK, GREEN);
  SET_PIX(left_offset + 4, 2, 'r', BLACK, GREEN);
  SET_PIX(left_offset + 5, 2, 'e', BLACK, GREEN);
  SET_PIX(left_offset + 6, 2, ':', BLACK, GREEN);
  SET_PIX(left_offset + 7, 2, ' ', BLACK, GREEN);
  SET_PIX(left_offset + 8, 2, ' ', BLACK, GREEN);

  if (score >= 100) {
    SET_PIX(left_offset + 9, 2, (unsigned char)((int)('0') + (score / 100)), BLACK, GREEN);
  } else {
    SET_PIX(left_offset + 9, 2, ' ', BLACK, GREEN);
  }
  if (score >= 10) {
    SET_PIX(left_offset + 10, 2, (unsigned char)((int)('0') + ((score%100) / 10)), BLACK, GREEN);
  } else {
    SET_PIX(left_offset + 10, 2, ' ', BLACK, GREEN);
  }
  SET_PIX(left_offset + 11, 2, (unsigned char)((int)('0') + (score%10)), BLACK, GREEN);
}
...
void display (GameState* gs) {
  ...
  draw_health(gs->left.health, 0);
  draw_score(gs->left.score, 0);
  draw_health(gs->right.health, WIDTH/2);
  draw_score(gs->right.score, WIDTH/2);
  draw_border();
}
